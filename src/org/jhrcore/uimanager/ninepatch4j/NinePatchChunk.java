 ...
 */
package org.jhrcore.uimanager.ninepatch4j;
...
public class NinePatchChunk implements Serializable {
    ...
    private void findPatches(BufferedImage image) {
        ...
        if (this.mFixed.size() > 0) {
            ...
            this.mVerticalPatches = getVerticalRectangles(height,
                    (List) top.mFirst);
        } else if (((List) left.mFirst).size() > 0) {
            this.mHorizontalPatches = getHorizontalRectangles(width,
                    (List) left.mFirst);
    ...
    private Pair<Integer> getPadding(List<Pair<Integer>> pairs) {
        ...
        if (pairs.size() == 1) {
            if (((Integer) ((Pair) pairs.get(0)).mFirst).intValue() == 0) {
                return new Pair(
                        Integer.valueOf(((Integer) ((Pair) pairs.get(0)).mSecond).intValue()
                        - ((Integer) ((Pair) pairs.get(0)).mFirst).intValue()), Integer.valueOf(0));
            }
            return new Pair(Integer.valueOf(0),
                    Integer.valueOf(((Integer) ((Pair) pairs.get(0)).mSecond).intValue()
                    - ((Integer) ((Pair) pairs.get(0)).mFirst).intValue()));
        }
        ...
        return new Pair(
                Integer.valueOf(((Integer) ((Pair) pairs.get(0)).mSecond).intValue()
                - ((Integer) ((Pair) pairs.get(0)).mFirst).intValue()),
                Integer.valueOf(((Integer) ((Pair) pairs.get(index)).mSecond).intValue()
                - ((Integer) ((Pair) pairs.get(index)).mFirst).intValue()));
    }
