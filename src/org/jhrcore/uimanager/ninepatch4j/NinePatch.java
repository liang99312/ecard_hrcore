...
public class NinePatch {
    ...
    throws IOException
  {
    ...
    boolean is9Patch = fileUrl.getPath().toLowerCase().endsWith(
      ".9.png");
  ...
  {
    this.mChunk.draw(this.mImage, graphics2D, x, y, scaledWidth, 
      scaledHeight, 0, 0);
  }
  ...
  private static BufferedImage convertTo9Patch(BufferedImage image)
  {
    BufferedImage buffer = 
      GraphicsUtilities.createTranslucentCompatibleImage(image.getWidth() + 2, 
      image.getHeight() + 2);
  ...
  private BufferedImage extractBitmapContent(BufferedImage image) {
    return image.getSubimage(1, 1, image.getWidth() - 2, image
      .getHeight() - 2);
  }
}
